...
class ObjStore(Tool, metaclass=Singleton):
    ...
    def setup(self, ctx: Context) -> "ObjStore":
        ...
        if None in (ctx.state.objstore.endpoint,
    ...
    def authenticate(self, ctx: Context, version: Version, *args: list[str]) -> Invocation:
        if len(args) == 4:
    ...
    @Tool.action("ObjStore")
    def lookup(self, ctx: Context, version: Version, path: str) -> Invocation:
        ObjStore().setup(ctx)
        if lkp := ObjStore().get_info(path):
            logging.info(pprint.pformat(lkp,
                                   indent=4,
                                   compact=True,
